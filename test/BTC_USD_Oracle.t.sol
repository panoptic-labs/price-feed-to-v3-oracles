...
contract PythToV3OracleTest is Test {
    ...
    // https://app.uniswap.org/explore/pools/unichain/0x53b06f1bb8b622cc4b7dbd9bc9f4a34788034bc48702cd2af4135b48444d5b24
    PoolId btcUsdcv4PoolId = PoolId.wrap(0x53b06f1bb8b622cc4b7dbd9bc9f4a34788034bc48702cd2af4135b48444d5b24);

    // https://app.uniswap.org/explore/pools/unichain/0xc349e9692b4afe1bcfdd6fadaf9ff0df2a2bea8c1a3e56323b57be08e4b8df6a
    PoolId btcUsdtv4PoolId = PoolId.wrap(0xc349e9692b4afe1bcfdd6fadaf9ff0df2a2bea8c1a3e56323b57be08e4b8df6a);
    ...
    function setUp() public {
        ...
        oracle =
            new PythToV3Oracle(pyth, btcUsdPriceFeedId, maxPythPriceAge, decimalDifferenceFromToken0ToToken1, shouldInvert);
    }
    ...
    function testSlot0ReturnsValidPrice() public {
        ...
        // Basic sanity checks
        assertLt(int256(tick), 0, "tick should be < 0, unless ETH broke 10^10 BTC");
    ...
    function testPriceComparisonWithUniswapPool() public {
        ...
        (, int24 usdcPoolTick,,) = stateView.getSlot0(btcUsdcv4PoolId);

        console.log(usdcPoolTick);

        (, int24 usdtPoolTick,,) = stateView.getSlot0(btcUsdtv4PoolId);

        console.log(usdtPoolTick);

        uint256 oraclePrice = convertRawUnitTickToWholeUnitPrice(oracleTick);
        uint256 usdcPoolPrice = convertRawUnitTickToWholeUnitPrice(usdcPoolTick);
        uint256 usdtPoolPrice = convertRawUnitTickToWholeUnitPrice(usdtPoolTick);

        console.log(oraclePrice);
        console.log(usdcPoolPrice);
        console.log(usdtPoolPrice);

        uint256 usdcPriceDiff = oraclePrice > usdcPoolPrice ? oraclePrice - usdcPoolPrice : usdcPoolPrice - oraclePrice;
        uint256 usdtPriceDiff = oraclePrice > usdtPoolPrice ? oraclePrice - usdtPoolPrice : usdtPoolPrice - oraclePrice;

        // Check if difference is within 1% (priceDiff / poolPrice < 0.01)
        // priceDiff / poolPrice < 0.01 <=> priceDiff * 100 < poolPrice
        assertLt(usdcPriceDiff * 100, usdcPoolPrice, "Oracle price should be within 1% of USDC pool price");
        assertLt(usdtPriceDiff * 100, usdtPoolPrice, "Oracle price should be within 1% of USDT pool price");
    }

    function convertRawUnitTickToWholeUnitPrice(int24 tick) internal pure returns (uint256) {
        uint160 sqrtPX96 = TickMath.getSqrtRatioAtTick(tick);

        // Adjust decimals based on your token pair
        // For BTC (8 decimals) / USDC (6 decimals)
        uint256 price = FullMath.mulDiv(
            uint256(sqrtPX96) * uint256(sqrtPX96),
            10 ** (6 + 18), // USDC decimals + precision buffer
            (1 << 192) * 10 ** 8 // Q96^2 * BTC decimals
        );
    ...
    function testRevertOnStalePriceAndAcceptsAllOthers(uint256 secondsInFuture) public {
        ...
        PythStructs.Price memory price = pyth.getPriceUnsafe(btcUsdPriceFeedId);
